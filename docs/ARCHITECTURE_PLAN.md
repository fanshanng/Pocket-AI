...
- `v1.3.6`: Move the drawer drag recognizer to `react-native-gesture-handler` and drive the shared canvas with Reanimated shared values for UI-thread finger tracking while keeping JS fallback snaps.
